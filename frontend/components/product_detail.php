    ...
    <main class="product-detail">
        <style>
            ...
            .pd-related-title {
                ...
                margin: 40px 0 20px;
                padding-bottom: 10px;
                border-bottom: 2px solid #0b59d6;
        ...
        <?php else: ?>
            <div class="product-detail-wrapper">
                ...
                <div class="pd-main">
                    <?php
                    // Xử lý lại đường dẫn ảnh cho chính xác
                    $rawThumb = $product['thumbnail'] ?? '';
                    if (!empty($rawThumb)) {
                        // Nếu DB đã lưu sẵn đường dẫn (có dấu /), dùng luôn
                        if (strpos($rawThumb, '/') !== false) {
                            $thumbnail = $rawThumb;
                        } else {
                            // Nếu DB chỉ lưu tên file (vd: hat_cho.png), cộng thêm thư mục vào
                        ...
                        <div class="pd-benefits">
                            <div class="pd-benefit-item">
                                ...
                                <div>
                                    ...
                                    <div class="pd-benefit-desc">Nội thành Tp. Cần Thơ</div>
                                </div>
                            </div>
                            <div class="pd-benefit-item">
                                <div class="pd-benefit-icon">⭐</div>
                        ...
                        <?php foreach ($relatedProducts as $relProd): ?>
                            <div class="product-card" style="border: 1px solid #eee; border-radius: 8px; overflow: hidden; background: #fff; transition: box-shadow 0.2s;">
                                <a href="?id=<?php echo (int) $relProd['product_id']; ?>" style="display: block; padding: 10px; text-align: center;">
                                    ...
                                    $rawRelThumb = $relProd['thumbnail'] ?? '';
                                    if (!empty($rawRelThumb)) {
                                        if (strpos($rawRelThumb, '/') !== false) {
                                            $relThumbnail = $rawRelThumb;
                                        } else {
                                            // Nối thêm thư mục chứa ảnh
                                            $relThumbnail = '/PetsAccessories/admin/backend/uploads/products/' . $rawRelThumb;
                                        }
                                    } else {
                                        $relThumbnail = '/PetsAccessories/frontend/public/images/default.jpg';
                                    }
                                    ?>
                                    <img src="<?php echo htmlspecialchars($relThumbnail); ?>"
