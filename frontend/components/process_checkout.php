            ...
            <div style="margin: 0 auto 25px; max-width: 520px; text-align: left; background: #fff; border: 1px solid #eee; border-radius: 10px; padding: 14px;">
                ...
                <?php foreach ($lineItems as $it): ?>
                    <div style="display:flex; gap:10px; align-items:center; padding: 10px 0; border-bottom: 1px dashed #eee;">
                        <?php
                        // Xử lý đường dẫn ảnh cho trang xác nhận đơn hàng
                        $rawThumb = $it['thumbnail'] ?? '';
                        if (!empty($rawThumb)) {
                            if (strpos($rawThumb, '/') !== false) {
                                $thumbnailUrl = $rawThumb;
                            } else {
                                // Nối thêm đường dẫn thư mục ảnh
                                $thumbnailUrl = '/PetsAccessories/admin/backend/uploads/products/' . $rawThumb;
                            }
                        } else {
                            $thumbnailUrl = '/PetsAccessories/frontend/public/images/default.jpg';
                        }
                        ?>
